The sandbox for this challenge is being prepared. Once it's ready, click the *Start* button.

In this final challenge, you'll:

- Run the full Aspire stack and verify all resources are healthy.
- Start the Diagrid Dev Dashboard alongside the Aspire solution.
- Start a Dapr Workflow and inspect its state and execution history in the Diagrid Dev Dashboard.

If you have any questions or feedback about this track, you can let us know in the *#general* channel of the [Dapr Discord server](https://diagrid.ws/dapr-discord).
